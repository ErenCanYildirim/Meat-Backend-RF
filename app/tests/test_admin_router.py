...
class MockUser:
    def __init__(self, id=1, email="test@example.com", company_name="Test Company", roles=None, admin=False):
        self.id = id
        self.email = email
...
class TestGetMyProfile:
    ...
    def test_get_my_profile_success_regular_user(self, client_with_regular_user, regular_user):
    ...
    def test_get_my_profile_multiple_roles(self, app):
        ...
        @router.get("/me")
        async def get_my_profile(current_user = Depends(get_mock_current_user(multi_role_user))):
            return {
                "user": {
                    "id": current_user.id,
                    "email": current_user.email,
    ...
    def test_get_my_profile_no_roles(self, app):
        ...
        @router.get("/me")
        async def get_my_profile(current_user = Depends(get_mock_current_user(no_role_user))):
            return {
                "user": {
                    "id": current_user.id,
                    "email": current_user.email,
...
class TestEdgeCases:
    ...
    def test_profile_with_none_roles(self, app):
        ...
        @router.get("/me")
        async def get_my_profile(current_user = Depends(get_mock_current_user(user_with_none_roles))):
            return {
                "user": {
                    "id": current_user.id,
                    "email": current_user.email,
