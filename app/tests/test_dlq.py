...
def test_single_failure():
    ...
    try:
        ...
        order_data = create_test_order_data(
            order_id=9001, customer_email="test.fail@example.com"
        )
...
def test_timeout_scenario():
    ...
    try:
        pdf_queue = get_pdf_queue()
        order_data = create_test_order_data(
            order_id=9002, customer_email="test.timeout@example.com"
        )
...
def test_various_failures():
    ...
    try:
        ...
        for i, failure_mode in enumerate(failure_modes):
            order_data = create_test_order_data(
                order_id=9010 + i, customer_email=f"test.{failure_mode}@example.com"
            )
