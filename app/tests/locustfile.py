    ...
    def get_current_user(self):
        ...
        with self.client.get(
            ...
            if response.status_code == 200:
                ...
                response.failure(f"Get current user failed with status {response.status_code}")
    ...
    def test_duplicate_email_registration(self):
        ...
        if self.user_data:
            ...
            with self.client.post(
                ...
                if response.status_code == 200:
                    response_data = response.json()
                    if "error" in response_data and "already registered" in response_data["error"]:
                        response.success()
                    else:
                        response.failure("Expected duplicate email error not returned")
                else:
                    response.failure(f"Duplicate email test failed with status {response.status_code}")
    ...
    def test_duplicate_company_registration(self):
        ...
        if self.user_data:
            ...
            with self.client.post(
                ...
                if response.status_code == 200:
                    response_data = response.json()
                    if "error" in response_data and "already taken" in response_data["error"]:
                        response.success()
                    else:
                        response.failure("Expected duplicate company error not returned")
                else:
                    response.failure(f"Duplicate company test failed with status {response.status_code}")
    ...
    def test_invalid_login(self):
        ...
        with self.client.post(
            ...
            if response.status_code == 200:
                ...
                response.failure(f"Invalid login test failed with status {response.status_code}")
