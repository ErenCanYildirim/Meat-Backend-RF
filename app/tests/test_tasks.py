...
def generate_pdf_task_with_failures(
    order_data: Dict[str, Any], failure_mode: str = "none"
) -> str:
    ...
    try:
        ...
        if failure_mode == "always":
            ...
            raise ConnectionError(
                "Simulated network error - could not reach PDF service"
            )
...
def send_email_task_with_failures(
    order_data: Dict[str, Any], pdf_filename: str, failure_mode: str = "none"
) -> bool:
    ...
    try:
        if failure_mode == "always":
            ...
            raise Exception(
                f"Invalid recipient email: {order_data.get('customer_email', 'unknown@example.com')}"
            )
...
def create_test_order_data(
    order_id: int, customer_email: str = "test@example.com"
) -> Dict[str, Any]:
    return {
        "order_id": order_id,
