...
from app.tests.test_dlq import (
    print_queue_status,
)
                ...
                try:
                    if job.args and len(job.args) > 0:
                        ...
                        if isinstance(job_data, dict):
                            print(f"Original function: {job_data.get('original_func', 'Unknown')}")
                            print(f"Original job ID: {job_data.get('original_job_id', 'Unknown')}")
                            print(f"Failure reason: {job_data.get('failure_reason', 'Unknown')}")
